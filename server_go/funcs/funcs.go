...
func CreateUser() {
	...
	CheckError(err)
	defer db.Close()
	username := Encrypt("New_user", string(SetKey()))
	rolename := Encrypt("doctor", string(SetKey()))
	realname := Encrypt("Имя", string(SetKey()))
	passhash := sha256.New()
	passhash.Write([]byte("1234"))
	hashValue := passhash.Sum(nil)
	result, err := db.Exec(`insert into users (username,password,realname,role) values ($1,$2,$3,$4)`, username, hashValue, realname, rolename)
	_ = result
	_ = err
